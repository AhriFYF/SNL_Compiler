...
SymbolNode *parseSymbolTable(const string &filePath)
{
    ...
    while (getline(file, line))
    {
        ...
        else if (line.find("PROCEDURE") != string::npos)
        {                                                                                        // 过程处理
        ...
        { // 参数处理       //Parameter	Type: INTEGER | varKind | dir | Name: i | Offset: 0
        ...
        { // 数组处理       //Array	Type: INTEGER | varKind | dir | Name: b | Offset: 2
        ...
        { // 变量处理      //Type: INTEGER | varKind | dir | Name: v1 | Offset: 0
...
void semanticAnalysis(Node *node, SymbolTable *symTable, ofstream &outputFile, SymbolNode *Parsedsymboltable, int depth, int childnum)
{
    ...
    if (depth == enterdepth && enterdepth != 0)
    {                                 // 过程声明节点
        enterdepth = 0;               // 重置层级深度
    ...
    { // 语句节点
        ...
        if (node->name == "const")
        {
            ...
            else
            {
                ...
                while (node->parent && !node->parent->children.empty() && k < node->parent->children.size())
                {
                    ...
                    SymbolEntry *entry2 = nullptr;
                    if (node->parent->children[k]->name != "const" && node->parent->children[k]->name != "OP" && node->parent->children[k]->name != "[" && node->parent->children[k]->name != "]" && node->parent->children[k]->name != "THEN" && node->parent->children[k]->name != "ELSE" && !isNumberSimple(node->parent->children[k]->name))
                    {
        ...
        if (isassignk == 1)
        {
            ...
            if (node->parent && node->parent->name == "AssignK")
            {
                ...
                if (node->parent->children[0]->name != "const" && node->parent->children[0]->name != "OP" && node->parent->children[0]->name != "[" && node->parent->children[0]->name != "]" && !isNumberSimple(node->parent->children[0]->varType))
                {
                ...
                while (node->parent && !node->parent->children.empty() && k < node->parent->children.size())
                {
                    ...
                    SymbolEntry *entry2 = nullptr;
                    if (node->parent->children[k]->name != "const" && node->parent->children[k]->name != "OP" && node->parent->children[k]->name != "[" && node->parent->children[k]->name != "]" && !isNumberSimple(node->parent->children[k]->name))
                    {
                        if (!symTable->FindEntry(childNode->name, "one", &entry2))
                        {
                            if (!symTable->FindEntry(childNode->name, "up", &entry2))
        ...
        if (iscallk == 1)
        {
            ...
            if (node->parent && node->parent->name == "CALL")
            {
                ...
                while (node->parent && !node->parent->children.empty() && k < node->parent->children.size())
                {
                    ...
                    if (node->parent && node->parent->children[k]->name != "const" && node->parent && node->parent->children[k]->name != "OP" && node->parent && node->parent->children[k]->name != "[" && node->parent && node->parent->children[k]->name != "]" && !isNumberSimple(node->parent->children[k]->varType))
                    {
                        if (!symTable->FindEntry(childNode->name, "one", &entry2))
                        {
                            if (!symTable->FindEntry(childNode->name, "up", &entry2))
        ...
        if (isconditonk == 1)
        {
            ...
            if (node->parent && node->parent->name == "IF" && node->parent->name == "WHILE")
            {
                // 检查表示式是否为bool类型
                int k = 0; // 从第零个节点开始
                while (node->parent && !node->parent->children.empty() && k < node->parent->children.size())
                {
                    ...
                    if (node->parent && node->parent->children[k]->name != "const" && node->parent && node->parent->children[k]->name != "OP" && node->parent && node->parent->children[k]->name != "[" && node->parent && node->parent->children[k]->name != "]" && node->parent && node->parent->children[k]->name != "THEN" && node->parent && node->parent->children[k]->name != "ELSE" && !isNumberSimple(node->parent->children[k]->varType))
                    {
